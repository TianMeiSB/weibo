        ...
        <div style="margin-left: 45%;margin-right: 45%;margin-top: 6%;float: left;">
            <p style="width:100px;height: 100px">
                <a href="#" id="a">
                    ...
                    <img src="${byUserId.image}" alt="..." class="img-circle" style="width: 100%;height: 100%;border: 1px solid #9F9F9F">
                </a>
            </p>
        </div>
        <p style="font-size: 28px;">${byUserId.username}</p>
        <p>${byUserId.introduce}</p>
        <c:if test="${follow==null}">
            <button type="button" class="layui-btn layui-btn-warm" style="width: 90px;margin-right: 40px;margin-top: 10px;" data-userid="${user.id}" data-followuserid="${byUserId.id}" onclick="guanzhu(this);">关注</button>
        </c:if>
        <c:if test="${follow!=null}">
        <button type="button" class="layui-btn layui-btn-normal" style="width: 90px;margin-right: 40px;margin-top: 10px;" data-userid="${user.id}" data-followuserid="${byUserId.id}" onclick="guanzhu(this);">取消关注</button>
        </c:if>
        <button type="button" class="layui-btn layui-btn-danger" style="width: 90px;margin-top: 10px;">举报</button>
    </div>
    <div class="layui-tab" lay-filter="test" style="width: 930px; margin: auto;background-color: #F8F8F8; margin-bottom: 1%;">
        <div style="margin-left: 40%;">
                <ul class="layui-tab-title">
                    <li lay-id="22" class="layui-this"><a href="/Index?userId=${byUserId.id}" style="text-decoration:none">主页</a></li>
                    <li></li>
                    <li lay-id="33"><a  style="text-decoration:none" href="/album?userId=${byUserId.id}">相册</a></li>
                </ul>
        </div>
    </div>
    ...
    <div style="width: 930px;margin: auto;">
        <div style="width: 33%;height: 100%;float: left;">
            <div style="width: 98%;height: 66px;margin: 3px;background-color: #F8F8F8;">
                <div style="width: 33%;height: 100%;text-align: center;float: left;border-right: 1px solid #DDDDDD"><div STYLE="margin-top: 12%"><a href="javascript:;" style="text-decoration:none" onclick="guanzhu2();">${guanzhu.size()}</a><p>关注</p></div></div>
            ...
            <div style="width: 98%;height: 230px;margin: 3px;background-color: #F8F8F8;margin-top: 10px" id="test">
                <div style="width: 100%;height: 70%;border-bottom: 1px solid #DDDDDD">
                    <table class="table">
                        <tr><td>用户名</td><td>${byUserId.username}</td></tr>
                        <tr><td>性别</td><td>${byUserId.sex}</td></tr>
                        <tr> <td>地址</td><td>${byUserId.address}</td></tr>
                        <tr><td>注册时间</td><td><fmt:formatDate value="${byUserId.registerTime}" pattern="yyyy-MM-dd hh:mm:ss"/></td></tr>
                    </table>
                </div>
            </div>
        </div>
            ...
            <div style="width: 99%;height:50px;background-color: #F8F8F8;margin: 3px;">
                ...
                <div style="float: right;">
                    <form class="navbar-form navbar-left">
                        <div class="form-group">
                            <input type="text" class="form-control" placeholder="搜索ta的微博">
                        </div>
                        <button type="submit" class="btn btn-default">GO</button>
                    </form>
                </div>
            ...
            <c:forEach items="${blogList}" var="blogList">
                ...
                <div>
                    ...
                    <div style="width: 99%;height: 45px;background-color: #FFFFFF;margin: 3px;margin-top: 0;">
                        <div style="width: 25%;height: 100%;float: left;text-align: center;border: 1px solid #DDDDDD;border-right: none;background-color: #F8F8F8"><div style="margin-top: 7%;"><a href="${ctx}/seeBlog?blogId=${blogList.id}" style="text-decoration:none;"><p><i class="layui-icon layui-icon-rate"></i> 收藏</p></a></div></div>
                        <div style="width: 25%;height: 100%;float: left;text-align: center;border: 1px solid #DDDDDD;border-right: none;"><div style="margin-top: 7%;"><a href="javascript:;" style="text-decoration:none"><i class="layui-icon layui-icon-share"></i> 分享</a></div></div>
                        ...
                        <div style="width: 25%;height: 100%;float: left;text-align: center;border: 1px solid #DDDDDD"><div style="margin-top: 7%;"><a href="${ctx}/seeBlog?blogId=${blogList.id}" style="text-decoration:none"><i class="layui-icon layui-icon-praise"></i> 点赞</a></div></div>
                    </div>
                </div>
            </c:forEach>
        </div>
...
<script>
    ...
    function guanzhu(obj){
        ...
        var followUserId = obj.dataset.followuserid;
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("POST","${ctx}/addFollow",true);
        xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        xmlhttp.send("userId="+userId+"&followUserId="+followUserId);
    ...
    function fensi() {
        ...
        $("#test").html('<div style="width: 100%;height: 15%;border-bottom: 1px solid #DDDDDD;float: left;text-align: center">\n' +
            '                    <p style="color: #3c9cea;margin-top: 2%">他的粉丝</p>\n' +
            '                </div>\n' +
    ...
    function guanzhu2() {
        $("#test").empty();
        $("#test").html('<div style="width: 100%;height: 15%;border-bottom: 1px solid #DDDDDD;float: left;text-align: center">\n' +
            '                    <p style="color: #3c9cea;margin-top: 2%">他的关注</p>\n' +
            '                </div>\n' +
            '                <div style="width: 100%;height: 85%;float: left;overflow-y: auto;">\n' +
